...
class Conv4D(hk.Module):
    def __init__(self,
        ...
        self.conv3dlist = [hk.Conv3D(output_channels,
    ...
    def __call__(self, inputs):
        ...
        for i_kernel in range(l_k):  # kernel
            for j_input in range(l_i):  # input
                ...
                if self.data_format == 'NCLDHW':
                    ...
                    channel_last_inputs = jnp.moveaxis(selected_inputs, source=1, destination=-1)
                else:
        ...
        if self.data_format == 'NCLDHW':
            output = jnp.moveaxis(output, source=5, destination=1)
        return output
...
if __name__ == '__main__':
    ...
    decoy_input = jnp.zeros(shape=(1, 5, 16, 17, 18, 1))


    # One needs to transform the function following the haiku framework.
    def my_conv_forward_fn(x):
        # data formats = NLDHWC or NCLDHW
        module = Conv4D(output_channels=8, kernel_shape=(4, 3, 3, 3), stride=(2, 1, 1, 1),
                        data_format='NCLDHW',
                        padding=((3, 0), (0, 0), (0, 0), (0, 0)))
        return module(x)


    my_forward = hk.transform(my_conv_forward_fn)
    rng_key = jax.random.PRNGKey(42)
    params = my_forward.init(rng=rng_key, x=decoy_input)

    n_iter = 20
    # First let's look at the uncompiled performance. We will look at it again, because the GPU needs to heat.
    t_0 = time.perf_counter()
    for _ in range(n_iter):
        out = my_forward.apply(params=params, x=decoy_input, rng=rng_key)
    out.block_until_ready()
    print('Time with an uncompiled model', time.perf_counter() - t_0)

    # Now let us compile our function
    my_forward_apply = jax.jit(my_forward.apply)
    t_0 = time.perf_counter()
    out = my_forward_apply(params=params, x=decoy_input, rng=rng_key)
    out.block_until_ready()
